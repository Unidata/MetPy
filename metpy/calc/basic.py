...
def altimeter_to_station_pressure(altimeter_value, height):
    ...
    a = lapse rate in NACA standard atmosphere below the isothermal layer 0.0065
       ^{\circ}C. m.^{-1})
    ...
    And solving for p_{mb} results in the equation below, which is used to
    calculate station pressure (p_{mb})
...
def altimeter_to_sea_level_pressure(altimeter_value, height, temperature):
    ...
    Equation 2.29
    .. math:: \Delta z = Z_{2} - Z_{1} = \frac{R_{d} \bar T_{v}}{g_0}ln\
    left(\frac{p_{1}}{p_{2}}\right) = \bar H ln \left (\frac {p_{1}}{p_{2}} \right)

    Equation 2.31
    .. math:: p_{0} = p_{g}exp \left(\frac{Z_{g}}{\bar H} \right) = p_{g} exp
    \left(\frac{g_{0}Z_{g}}{R_{d}\bar T_{v}} \right)
