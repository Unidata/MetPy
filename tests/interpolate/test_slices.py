...
def test_ds_xy():
    ...
    ds["temperature"].attrs = {"grid_mapping": "lambert_conformal"}
...
def test_interpolate_to_slice_against_selection(test_ds_lonlat):
    ...
    path = np.array([[265.0, 30.0], [265.0, 36.0], [265.0, 42.0]])
    test_slice = interpolate_to_slice(data, path)
    true_slice = data.sel({"lat": [30.0, 36.0, 42.0], "lon": 265.0})
...
def test_geodesic(test_ds_xy):
    ...
    truth = np.array(
        [
            [-4.99495719e05, -1.49986599e06],
            [9.84044354e04, -1.26871737e06],
            [6.88589099e05, -1.02913966e06],
            [1.27269045e06, -7.82037603e05],
            [1.85200974e06, -5.28093957e05],
            [2.42752546e06, -2.67710326e05],
            [2.99993290e06, -9.39107692e02],
        ]
    )
...
def test_cross_section_dataset_and_nearest_interp(test_ds_lonlat):
    ...
    nearest_values = test_ds_lonlat.isel(
        lat=xr.DataArray([0, 1, 2, 3, 3, 4, 5], dims="index"),
    ...
    truth_values_lon = np.array(
        [255.5, 258.20305939, 261.06299342, 264.10041516, 267.3372208, 270.7961498, 274.5]
    )
    truth_values_lat = np.array(
        [30.5, 33.02800969, 35.49306226, 37.88512911, 40.19271688, 42.40267088, 44.5]
    )
    index = xr.DataArray(range(7), name="index", dims=["index"])
