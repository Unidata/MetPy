...
def test_var_multidim_full(test_ds):
    ...
    return (
        test_ds[{"isobaric": [6, 12], "y": [95, 96], "x": [122, 123]}]
        .squeeze()
...
def test_globe(test_var, ccrs):
    ...
    assert globe.to_proj4_params() == OrderedDict(
        [("ellps", "sphere"), ("a", 6367470.21484375), ("b", 6367470.21484375)]
    )
...
def test_dataset_dequantify():
    ...
    original = xr.Dataset(
        {"test": ("x", units.Quantity([280, 290, 300], "K")), "x": np.arange(3)}
    )
    result = original.metpy.dequantify()
...
def test_radian_projection_coords():
    ...
    proj = xr.DataArray(
        0, attrs={"grid_mapping_name": "geostationary", "perspective_point_height": 3}
    )
    x = xr.DataArray(
        np.arange(3), attrs={"standard_name": "projection_x_coordinate", "units": "radians"}
    )
    y = xr.DataArray(
        np.arange(2), attrs={"standard_name": "projection_y_coordinate", "units": "radians"}
    )
    data = xr.DataArray(
...
def test_missing_grid_mapping():
    ...
    lon = xr.DataArray(
        -np.arange(3), attrs={"standard_name": "longitude", "units": "degrees_east"}
    )
    lat = xr.DataArray(
        np.arange(2), attrs={"standard_name": "latitude", "units": "degrees_north"}
    )
...
def test_coordinates_specified_by_name_with_dataset(test_ds_generic):
    ...
    data = test_ds_generic.metpy.parse_cf(
        coordinates={"time": "a", "vertical": "b", "y": "c", "x": "d"}
    )
...
def test_check_matching_coordinates(test_ds_generic):
    ...
    xr.testing.assert_identical(
        add(test_ds_generic["test"], test_ds_generic["test"]), test_ds_generic["test"] * 2
    )
...
def test_data_array_loc_set_with_units(test_var):
    ...
    assert np.isnan(temperature.loc[:, 850.0]).all()
    assert not np.isnan(temperature.loc[:, 700.0]).any()
...
def test_data_array_sel_dict_with_units(test_var):
    ...
    truth = test_var.squeeze().loc[500.0]
    assert truth.identical(
        test_var.metpy.sel({"time": "1987-04-04T18:00:00", "isobaric": 5e4 * units.Pa})
    )


def test_data_array_sel_kwargs_with_units(test_var):
    """Test .sel on the metpy accessor with kwargs and axis type."""
    truth = test_var.loc[:, 500.0][..., 122]
    selection = test_var.metpy.sel(
        vertical=5e4 * units.Pa, x=-16.569 * units.km, tolerance=1.0, method="nearest"
    ).metpy.assign_coordinates(None)
...
def test_dataset_loc_with_units(test_ds):
    ...
    assert truth.identical(
        test_ds.metpy.loc[{"isobaric": slice(8.5e4 * units.Pa, 5e4 * units.Pa)}]
    )
...
def test_dataset_sel_mixed_dict_and_kwarg(test_ds):
    ...
    with pytest.raises(ValueError) as exc:
        test_ds.metpy.sel(
            {"isobaric": slice(8.5e4 * units.Pa, 5e4 * units.Pa)}, time="1987-04-04T18:00:00"
        )
...
def test_coordinate_identification_shared_but_not_equal_coords():
    ...
    temperature = xr.DataArray(
        ...
        coords={
            ...
            "x": xr.DataArray(
                [0, 450], name="x", dims="x", attrs={"units": "km", "axis": "X"}
            ),
        },
        attrs={"units": "degC"},
    )
...
def test_assign_latitude_longitude_no_horizontal(test_ds_generic):
    ...
    with pytest.warns(UserWarning):
        xr.testing.assert_identical(
            test_ds_generic, test_ds_generic.metpy.assign_latitude_longitude()
        )
...
def test_assign_y_x_no_horizontal(test_ds_generic):
    ...
    with pytest.warns(UserWarning):
        xr.testing.assert_identical(test_ds_generic, test_ds_generic.metpy.assign_y_x())
...
def test_coord_helper_da_dummy_latlon(test_coord_helper_da_yx):
    ...
    return test_coord_helper_da_yx.assign_coords(latitude=0.0, longitude=0.0)
    ...
    return xr.DataArray(
        ...
        coords={
            ...
            "longitude": xr.DataArray(
                np.array(
                    [
                        [-101.10219213, -100.55111288, -100.0],
                        [-101.10831414, -100.55417417, -100.0],
                        [-101.11450453, -100.55726965, -100.0],
                    ]
                ),
                dims=("y", "x"),
            ),
...
def test_assign_latitude_longitude_basic_dataarray(
    ...
    np.testing.assert_array_almost_equal(
        test_coord_helper_da_latlon["latitude"].values, lat.values, 3
    )
    np.testing.assert_array_almost_equal(
        test_coord_helper_da_latlon["longitude"].values, lon.values, 3
    )


def test_assign_latitude_longitude_error_existing_dataarray(test_coord_helper_da_dummy_latlon):
...
def test_assign_y_x_error_existing_dataarray(test_coord_helper_da_dummy_yx):
...
@needs_cartopy
def test_assign_y_x_dataset_assumed_order(
    test_coord_helper_da_yx, test_coord_helper_da_latlon
):
    """Test assign_y_x on Dataset where order must be assumed."""
    with pytest.warns(UserWarning):
        new_ds = (
            test_coord_helper_da_latlon.to_dataset(name="test")
            .rename_dims({"y": "b", "x": "a"})
            .metpy.assign_y_x()
        )
...
def test_assign_y_x_error_existing_dataset(test_coord_helper_da_dummy_yx):
...
def test_update_attribute_dictionary(test_ds_generic):
    ...
    descriptions = {"test": "Filler data", "c": "The third coordinate"}
...
@pytest.mark.parametrize(
    "test, other, match_unit, expected",
    [
        ...
        (
            ...
            True,
            xr.DataArray(
                np.arange(4) * units.meter, dims=("x",), coords={"x": np.linspace(0, 1, 4)}
            ),
        ),
        ...
        (
            [2, 4, 8] * units.kg,
            xr.DataArray(
                np.zeros(3) * units.meter, dims=("x",), coords={"x": np.linspace(0, 1, 3)}
            ),
            False,
            xr.DataArray(
                [2, 4, 8] * units.kilogram, dims=("x",), coords={"x": np.linspace(0, 1, 3)}
            ),
        ),
        (
            [2, 4, 8] * units.kg,
            xr.DataArray(
                np.zeros(3) * units.gram, dims=("x",), coords={"x": np.linspace(0, 1, 3)}
            ),
            True,
            xr.DataArray(
        ...
        (
            xr.DataArray(
                np.linspace(0, 1, 5) * units.meter, attrs={"description": "A range of values"}
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (xr.DataArray(np.linspace(0, 1, 5) * units.meter), [0] * units.kg),
        (
            xr.DataArray(np.linspace(0, 1, 5) * units.meter),
            xr.DataArray(
                np.zeros(5) * units.kg, dims=("x",), coords={"x": np.linspace(0, 1, 5)}
            ),
        ),
        (
            xr.DataArray(
...
def test_grid_deltas_from_dataarray_lonlat(test_da_lonlat):
    ...
    true_dy = (
        np.array(
            [
                [
                    [369603.78775948, 369603.78775948, 369603.78775948, 369603.78775948],
                    [369802.28173967, 369802.28173967, 369802.28173967, 369802.28173967],
                    [370009.56291098, 370009.56291098, 370009.56291098, 370009.56291098],
                ]
            ]
        )
        * units.m
...
def test_grid_deltas_from_dataarray_actual_xy(test_da_xy, ccrs):
    ...
    lonlat = ccrs.Geodetic(test_da_xy.metpy.cartopy_globe).transform_points(
        test_da_xy.metpy.cartopy_crs, x.values, y.values
    )
    lon = lonlat[..., 0]
    lat = lonlat[..., 1]
    ...
    true_dx = [
        [
            [
                [494426.3249766, 493977.6028005, 493044.0656467],
                [498740.2046073, 498474.9771064, 497891.6588559],
                [500276.2649627, 500256.3440237, 500139.9484845],
                [498740.6956936, 499045.0391707, 499542.7244501],
            ]
        ]
    ] * units.m
    true_dy = [
        [
            [
                ...
                [499750.8962991, 499826.2263137, 500004.4977747, 500150.9897759],
            ]
        ]
    ] * units.m
...
def test_grid_deltas_from_dataarray_lonlat_assumed_order():
    ...
    lat, lon = np.meshgrid(np.array([38.0, 40.0, 42]), np.array([263.0, 265.0, 267.0]))
    test_da = (
        xr.DataArray(
            ...
            coords={
                "lat": xr.DataArray(
                    lat, dims=("dim_0", "dim_1"), attrs={"units": "degrees_north"}
                ),
                "lon": xr.DataArray(
                    lon, dims=("dim_0", "dim_1"), attrs={"units": "degrees_east"}
                ),
            },
            attrs={"units": "K"},
        )
        .to_dataset()
        .metpy.parse_cf("temperature")
    )
    ...
    true_dy = [
        [175661.5413976, 170784.1311091, 165697.7563223],
        [175661.5413976, 170784.1311091, 165697.7563223],
    ] * units.m
    assert_array_almost_equal(dx, true_dx, 5)
    assert_array_almost_equal(dy, true_dy, 5)
...
def test_add_grid_arguments_from_dataarray():
    ...
    def return_the_kwargs(
        da, dz=None, dy=None, dx=None, vertical_dim=None, y_dim=None, x_dim=None, latitude=None
    ):
        return {
            "dz": dz,
            "dy": dy,
    ...
    data = (
        xr.DataArray(
            ...
            coords={
                ...
                "isobaric": (("isobaric",), [850.0, 700.0], {"units": "hPa"}),
                "lat": (("lat",), [30.0, 40.0], {"units": "degrees_north"}),
                "lon": (("lon",), [-100.0, -90.0], {"units": "degrees_east"}),
            },
        )
        .to_dataset(name="zeros")
        .metpy.parse_cf("zeros")
    )
    result = return_the_kwargs(data)
    assert_array_almost_equal(result["dz"], [-150.0] * units.hPa)
    ...
    assert_array_almost_equal(
        result["latitude"].metpy.unit_array, [30.0, 40.0] * units.degrees_north
    )
