...
@pytest.mark.mpl_image_compare(tolerance=0.0202, remove_text=True, style="default")
def test_skewt_api():
    """Test the SkewT API."""
...
@pytest.mark.mpl_image_compare(
    tolerance=0.0272 if matplotlib.__version__ < "3.2" else 34.4,
    remove_text=True,
    style="default",
)
...
@pytest.mark.mpl_image_compare(
    tolerance=0.0 if matplotlib.__version__ >= "3.2" else 30.0,
    remove_text=True,
    style="default",
)
...
@pytest.mark.skipif(
    matplotlib.__version__ < "3.2",
    reason="Matplotlib versions generate different image sizes.",
)
@pytest.mark.mpl_image_compare(
    tolerance=0.0, remove_text=False, style="default", savefig_kwargs={"bbox_inches": "tight"}
)
...
@pytest.mark.mpl_image_compare(tolerance=0.0, remove_text=True, style="default")
...
def test_hodograph_plot_colormapped():
    ...
    u = np.arange(5.0, 65.0, 5)
    v = np.arange(-5.0, -65.0, -5)
    u[3] = np.nan
    v[6] = np.nan
    fig = plt.figure(figsize=(9, 9))
    ax = fig.add_subplot(1, 1, 1)
...
def test_united_hodograph_range():
    ...
    fig = plt.figure(figsize=(6, 6))
    ax = fig.add_subplot(1, 1, 1)
    Hodograph(ax, component_range=60.0 * units.knots)
