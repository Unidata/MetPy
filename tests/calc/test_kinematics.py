...
def test_vorticity_asym():
    ...
    true_vort = (
        np.array([[-2.5, 3.5, 13.0], [8.5, -1.5, -11.0], [-5.5, -1.5, 0.0]]) / units.sec
    )
...
def test_zero_divergence():
    ...
    true_c = 2.0 * np.ones_like(u) / units.sec
...
def test_divergence_asym():
    ...
    true_c = np.array([[-2, 5.5, -2.5], [2.0, 0.5, -1.5], [3.0, -1.5, 8.5]]) / units.sec
    assert_array_equal(c, true_c)
...
def test_shearing_deformation_asym():
    ...
    true_sh = np.array([[-7.5, -1.5, 1.0], [9.5, -0.5, -11.0], [1.5, 5.5, 12.0]]) / units.sec
...
def test_stretching_deformation_asym():
    ...
    true_st = np.array([[4.0, 0.5, 12.5], [4.0, 1.5, -0.5], [1.0, 5.5, -4.5]]) / units.sec
...
def test_total_deformation_asym():
    ...
    true_tdef = (
        np.array(
            [
                [8.5, 1.58113883, 12.5399362],
                [10.30776406, 1.58113883, 11.0113578],
                [1.80277562, 7.7781746, 12.8160056],
            ]
        )
        / units.sec
    )
...
def test_frontogenesis_asym():
    ...
    true_fronto = (
        np.array(
            [
                [-52.4746386, -37.3658646, -50.3996939],
...
def test_advection_2d_asym():
    ...
    truth = np.array([[0, -20.75, -2.5], [-33.0, -16.0, 20.0], [-48, 91.0, 8]]) * units(
        "K/sec"
    )
    assert_array_equal(a, truth)
...
def test_geostrophic_wind():
    ...
    z = np.array([[48, 49, 48], [49, 50, 49], [48, 49, 48]]) * 10.0 * units.meter
    latitude = 30 * units.degrees
    ug, vg = geostrophic_wind(z.T, 100.0 * units.kilometer, 100.0 * units.kilometer, latitude)
...
def test_geostrophic_wind_asym():
    ...
    z = np.array([[1, 2, 4], [4, 8, 4], [8, 6, 4]]) * 20.0 * units.meter
    latitude = 30 * units.degrees
    ug, vg = geostrophic_wind(z, 2000.0 * units.kilometer, 1000.0 * units.kilometer, latitude)
    true_u = np.array(
...
def test_geostrophic_geopotential():
    ...
    z = np.array([[48, 49, 48], [49, 50, 49], [48, 49, 48]]) * 100.0 * units("m^2/s^2")
    latitude = 30 * units.degrees
    ug, vg = geostrophic_wind(z.T, 100.0 * units.kilometer, 100.0 * units.kilometer, latitude)
...
def test_geostrophic_3d():
    ...
    z = np.array([[48, 49, 48], [49, 50, 49], [48, 49, 48]]) * 10.0
    latitude = 30 * units.degrees
    z3d = np.dstack((z, z)) * units.meter
    ug, vg = geostrophic_wind(
        z3d.T, 100.0 * units.kilometer, 100.0 * units.kilometer, latitude
    )
    true_u = np.array([[-26.897, 0, 26.897]] * 3) * units("m/s")
    true_v = -true_u.T
...
def test_no_ageostrophic_geopotential():
    ...
    v = -u.T
    latitude = 30 * units.degrees
    uag, vag = ageostrophic_wind(
        z.T, u.T, v.T, 100.0 * units.kilometer, 100.0 * units.kilometer, latitude
    )
...
def test_streamfunc():
    ...
    t = 287.0 * units.kelvin
    hgt = 5000.0 * units.meter
...
def test_storm_relative_helicity_no_storm_motion():
    ...
    positive_srh, negative_srh, total_srh = storm_relative_helicity(
        heights, u, v, depth=750 * units.meters
    )

    assert_almost_equal(positive_srh, 400.0 * units("meter ** 2 / second ** 2 "), 6)
    assert_almost_equal(negative_srh, -100.0 * units("meter ** 2 / second ** 2 "), 6)
    assert_almost_equal(total_srh, 300.0 * units("meter ** 2 / second ** 2 "), 6)
...
def test_storm_relative_helicity_storm_motion():
    ...
    assert_almost_equal(pos_srh, 400.0 * units("meter ** 2 / second ** 2 "), 6)
    assert_almost_equal(neg_srh, -100.0 * units("meter ** 2 / second ** 2 "), 6)
    assert_almost_equal(total_srh, 300.0 * units("meter ** 2 / second ** 2 "), 6)
...
def test_storm_relative_helicity():
    ...
    srh_true_p = (0.25 * np.pi * (2 ** 2)) * units("m^2/s^2")
    ...
    p_srh, n_srh, t_srh = storm_relative_helicity(
        hgt_int,
        u_int,
        v_int,
        1000 * units("meter"),
...
def test_storm_relative_helicity_masked():
    ...
    pos, neg, com = storm_relative_helicity(
        h,
        u,
        v,
...
def test_absolute_vorticity_asym():
    ...
    true_vort = (
        np.array(
            [
                [-2.499927, 3.500073, 13.00007],
                [8.500050, -1.499950, -10.99995],
                [-5.499975, -1.499975, 2.532525e-5],
            ]
        )
        / units.sec
    )
...
def test_potential_vorticity_baroclinic_unity_axis0(pv_data):
    ...
    pvor = potential_vorticity_baroclinic(
        potential_temperature, pressure, u, v, dx[None, :, :], dy[None, :, :], lats[None, :, :]
    )

    abs_vorticity = absolute_vorticity(u, v, dx[None, :, :], dy[None, :, :], lats[None, :, :])

    vort_difference = pvor - (abs_vorticity * g * (-1 * (units.kelvin / units.hPa)))

    true_vort = np.zeros_like(u) * (
...
def test_potential_vorticity_baroclinic_isentropic_real_data():
    ...
    isentprs = (
        np.array(
            [
                [
                    [245.88052, 245.79416, 245.68776, 245.52525, 245.31844],
                ...
                [
                    ...
                    [233.89297, 233.66039, 233.49615, 233.38635, 233.35281],
                ],
            ]
        )
        * units.hPa
    ...
    isentu = (
        np.array(
            [
                [
                    [28.94226812, 28.53362902, 27.98145564, 27.28696092, 26.46488305],
    ...
    isentv = (
        np.array(
            [
                [
                    [-2.22336191, -2.82451946, -3.27190475, -3.53076527, -3.59311591],
    ...
    lats = np.array([57.5, 57.0, 56.5, 56.0, 55.5]) * units.degrees
    lons = np.array([227.5, 228.0, 228.5, 229.0, 229.5]) * units.degrees
...
def test_potential_vorticity_baroclinic_isobaric_real_data():
    """Test potential vorticity calculation with real isentropic data."""
    pres = [20000.0, 25000.0, 30000.0] * units.Pa
    theta = (
        np.array(
            [
                [
                    [344.45776, 344.5063, 344.574, 344.6499, 344.735],
    ...
    uwnd = (
        np.array(
            [
                [
                    [25.309322, 25.169882, 24.94082, 24.61212, 24.181437],
    ...
    vwnd = (
        np.array(
            [
                [
                    [-0.3050951, -0.90105104, -1.4307652, -1.856761, -2.156073],
...
def test_inertial_advective_wind_diffluent():
    ...
    lons = (
        np.array(
            [
                [250.0, 254.0, 258.0, 262.0, 266.0, 270.0, 274.0, 278.0, 282.0, 286.0, 290.0],
                [250.0, 254.0, 258.0, 262.0, 266.0, 270.0, 274.0, 278.0, 282.0, 286.0, 290.0],
                [250.0, 254.0, 258.0, 262.0, 266.0, 270.0, 274.0, 278.0, 282.0, 286.0, 290.0],
                [250.0, 254.0, 258.0, 262.0, 266.0, 270.0, 274.0, 278.0, 282.0, 286.0, 290.0],
                [250.0, 254.0, 258.0, 262.0, 266.0, 270.0, 274.0, 278.0, 282.0, 286.0, 290.0],
...
def q_vector_data():
    ...
    speed = np.ones((4, 4)) * 50.0 * units("knots")
    wdir = (
        np.array(
            [
                [210.0, 190.0, 170.0, 150.0],
                [190.0, 180.0, 180.0, 170.0],
                [170.0, 180.0, 180.0, 190.0],
    ...
    p = np.array([850.0, 700.0, 500.0]) * units("hPa")

    lats = np.linspace(35.0, 40.0, 4) * units("degrees")
    lons = np.linspace(-100.0, -90.0, 4) * units("degrees")
    dx, dy = lat_lon_grid_deltas(lons, lats)
...
def test_q_vector_without_static_stability(q_vector_data):
    ...
    q1_truth = (
        np.array(
            [
                [-2.7454089e-14, -3.0194267e-13, -3.0194267e-13, -2.7454089e-14],
    ...
    q2_truth = (
        np.array(
            [
                [-4.4976059e-14, -4.3582378e-13, 4.3582378e-13, 4.4976059e-14],
...
def test_q_vector_with_static_stability(q_vector_data):
    ...
    q1_truth = (
        np.array(
            [
                [-1.4158140e-08, -1.6197987e-07, -1.6875014e-07, -1.6010616e-08],
    ...
    q2_truth = (
        np.array(
            [
                [-2.3194263e-08, -2.3380160e-07, 2.4357380e-07, 2.6229040e-08],
...
def data_4d():
    ...
    subset = (
        ...
        .sel(
            latitude=[46.0, 44.0, 42.0, 40.0],
            longitude=[262.0, 267.0, 272.0, 277.0],
            isobaric3=[50000.0, 70000.0, 85000.0],
        )
        .isel(time1=[0, 1, 2])
    )
    ...
    truth = (
        np.array(
            [
                [
                    [
                        [-5.83650490e-05, 3.17327814e-05, 4.57268332e-05, 2.00732350e-05],
                ...
                [
                    ...
                    [
                        ...
                        [-2.96985363e-06, 3.86098537e-06, 5.24525482e-06, 2.72933874e-05],
                    ],
                ],
            ]
        )
    ...
    truth = (
        np.array(
            [
                [
                    [
                        [-8.43705083e-06, -5.42243991e-06, 1.42553766e-05, 2.81311077e-05],
                ...
                [
                    ...
                    [
                        ...
                        [9.23766788e-07, -2.85241737e-06, 1.68475020e-05, -5.70982211e-06],
                    ],
                ],
            ]
        )
    ...
    truth = (
        np.array(
            [
                [
                    [
                        [-2.32353766e-05, 3.38638896e-06, 2.68355706e-05, 1.06560395e-05],
                ...
                [
                    ...
                    [
                        ...
                        [1.23394357e-05, -1.98706807e-06, 1.56020711e-05, 3.89964205e-05],
                    ],
                ],
            ]
        )
    ...
    truth = (
        np.array(
            [
                [
                    [
                        [3.47764258e-05, 2.24655678e-05, -5.99204286e-06, -2.81311151e-05],
                ...
                [
                    ...
                    [
                        ...
                        [1.75811596e-05, 1.24562416e-05, -1.28654291e-05, 7.34949445e-06],
                    ],
                ],
            ]
        )
    ...
    truth = (
        np.array(
            [
                [
                    [
                        [4.18244250e-05, 2.27193611e-05, 2.74964075e-05, 3.00817356e-05],
                ...
                [
                    ...
                    [
                        ...
                        [2.14792655e-05, 1.26137383e-05, 2.02223611e-05, 3.96829419e-05],
                    ],
                ],
            ]
        )
...
def test_frontogenesis_4d(data_4d):
    ...
    frnt = frontogenesis(thta, data_4d.u, data_4d.v).transpose(
        "time1", "pressure", "latitude", "longitude"
    )
    truth = (
        np.array(
            [
                [
                    [
                        [4.23682195e-10, -6.42818314e-12, -2.16491106e-10, -3.81845902e-10],
                ...
                [
                    ...
                    [
                        ...
                        [-2.33302976e-11, 1.01795295e-10, 4.19754683e-12, 5.18286088e-10],
                    ],
                ],
            ]
        )
    ...
    u_g_truth = (
        np.array(
            [
                [
                    [
                        [4.40351577, 12.52087174, 20.6458988, 3.17057524],
                ...
                [
                    ...
                    [
                        ...
                        [2.97756024, 7.02083208, 8.67190524, 10.98516411],
                    ],
                ],
            ]
        )
    ...
    v_g_truth = (
        np.array(
            [
                [
                    [
                        [-2.34336304e01, -1.93589800e01, -7.42980465e00, 1.23538955e01],
                        [-2.05343103e01, -1.59281972e01, -7.22778771e00, 5.56691831e00],
                        [-2.12483061e01, -1.50276890e01, -1.26159708e00, 2.00499696e01],
                        [-2.82673839e01, -1.22322398e01, 2.74929719e00, 1.66772271e01],
                    ],
                    [
                        [-2.11572490e01, -1.57068398e01, -7.16428821e00, 4.47040576e00],
                        [-1.85233793e01, -1.38641633e01, -7.23745352e00, 1.35674995e00],
                        [-1.48069287e01, -1.29873005e01, -6.19402168e00, 5.57290769e00],
                        [-1.63708722e01, -1.07203268e01, -3.25405588e00, 6.02794043e00],
                    ],
                    [
                        [-1.83721994e01, -1.51434535e01, -8.30361332e00, 2.14732109e00],
                        [-1.60334603e01, -1.37004633e01, -8.52272656e00, -5.00249972e-01],
                        [-1.25811419e01, -1.30858045e01, -8.11893604e00, 2.31946331e00],
                        [-1.07972595e01, -1.12050147e01, -8.05482698e00, -1.34669618e00],
                    ],
                ],
                [
                    [
                        [-2.47128002e01, -2.06093912e01, -7.53605837e00, 1.45071982e01],
                        [-2.04618167e01, -1.66379272e01, -6.94777385e00, 8.60864325e00],
                        [-2.03847527e01, -1.41640171e01, -3.58588785e00, 1.13496351e01],
                        [-3.06442215e01, -1.34818877e01, 3.63145087e00, 2.06957942e01],
                    ],
                    [
                        [-2.20117576e01, -1.60592509e01, -6.79979611e00, 5.76660686e00],
                        [-1.88926841e01, -1.40452204e01, -7.10711240e00, 1.92164004e00],
                        [-1.49428085e01, -1.27155409e01, -6.56034626e00, 3.52277542e00],
                        [-1.56847892e01, -1.10535722e01, -3.82991450e00, 5.98618380e00],
                    ],
                    [
                        [-1.89418619e01, -1.48982095e01, -8.32871820e00, 7.66612047e-01],
                        [-1.57997569e01, -1.38337366e01, -9.12720817e00, -1.68017155e00],
                        [-1.34002412e01, -1.27868867e01, -8.32854573e00, -2.52183180e-02],
                        [-1.10305552e01, -1.16852908e01, -7.77451018e00, 7.01786569e-01],
                    ],
                ],
                [
                    [
                        [-2.87198561e01, -2.07541862e01, -7.39120444e00, 1.13690891e01],
                        [-2.50727626e01, -1.76277299e01, -6.48428638e00, 8.35756789e00],
                        [-2.15686958e01, -1.43774917e01, -4.66795064e00, 7.55992752e00],
                        [-3.08303915e01, -1.46678239e01, 2.17589132e00, 1.97007540e01],
                    ],
                    [
                        [-2.14034948e01, -1.55089179e01, -7.19566930e00, 3.53625110e00],
                        [-1.85643117e01, -1.42866005e01, -7.10227950e00, 2.98865138e00],
                        [-1.52824784e01, -1.23952994e01, -6.71565437e00, 1.75645659e00],
                        [-1.53343446e01, -1.06296646e01, -4.49885811e00, 3.05807491e00],
                    ],
                    [
                        [-1.62094234e01, -1.41161427e01, -9.20541452e00, -1.47723905e00],
                        [-1.41272620e01, -1.33895366e01, -9.16198151e00, -1.44459685e00],
                        [-1.29925870e01, -1.17892453e01, -8.27421454e00, -2.44749477e00],
                        [-1.08991833e01, -1.03581717e01, -7.35501458e00, -1.88971184e00],
                    ],
                ],
            ]
        )
    ...
    u_i_truth = (
        np.array(
            [
                [
                    [
                        [-4.74579332, -6.36486064, -7.20354171, -11.08307751],
                ...
                [
                    ...
                    [
                        ...
                        [-1.29544691, -0.9808466, -1.24778616, -1.95945874],
                    ],
                ],
            ]
        )
    ...
    v_i_truth = (
        np.array(
            [
                [
                    [
                        [1.03108918e01, 5.87304544e00, -3.23865690e00, -1.88225987e01],
                        [9.87187503e00, 5.33610060e00, 4.80874417e00, 3.92484555e-02],
                        [6.37856912e00, 6.46296166e00, 8.14267044e00, 4.37232518e00],
                        [-1.30385124e00, 1.01032585e01, 4.20243238e00, -1.97934081e01],
                    ],
                    [
                        [1.10360108e00, 2.30280536e00, -1.82078930e00, -3.54284012e00],
                        [2.43663102e00, 1.35818636e00, 4.92919838e-01, -9.85544117e-03],
                        [2.33985677e00, 1.03370035e00, 3.28069921e00, 4.50046765e-01],
                        [2.93689077e-01, 1.43848430e00, 6.69758269e00, -4.27897434e00],
                    ],
                    [
                        [4.77869846e-01, 1.14482717e00, -1.82404796e00, -1.95731131e00],
                        [5.19464097e-01, 4.52949199e-01, -3.26412809e-01, 6.88744088e-02],
                        [2.51097720e-01, 1.43448773e-01, 1.08982754e00, -9.69963394e-02],
                        [-3.37860948e-01, 2.48187099e-01, 2.41935519e00, -2.84847302e00],
                    ],
                ],
                [
                    [
                        [9.00342804e00, 6.74193832e00, 5.48141003e-01, -1.25005172e01],
                        [9.56628265e00, 4.57654669e00, 3.34479904e00, -7.13103555e00],
                        [5.46655351e00, 2.14241047e00, 7.51934330e00, 2.43229680e00],
                        [-5.48082957e00, -6.46852260e-01, 1.34334674e01, 1.61485491e01],
                    ],
                    [
                        [2.49375451e00, 3.34815514e00, -7.09673457e-01, -3.42185701e00],
                        [2.69963182e00, 1.64621317e00, 2.91799176e-01, -1.12584231e00],
                        [1.83462164e00, 1.71608154e-01, 1.87927013e00, 7.54482898e-01],
                        [-4.86175507e-01, -1.06374611e00, 4.20283383e00, 1.54789418e00],
                    ],
                    [
                        [1.05175282e00, 2.36715709e-01, -4.35406547e-01, -9.39935118e-01],
                        [5.26821709e-01, 1.34167595e-01, 6.74485663e-02, 1.18351992e-01],
                        [9.51152970e-02, 3.63519903e-02, 2.14587938e-01, 6.10557463e-01],
                        [-2.42904366e-01, -5.80309556e-02, -3.63185957e-02, 2.28010678e-01],
                    ],
                ],
                [
                    [
                        [5.18112516e00, 8.23347995e00, 2.85922078e00, -5.58457816e00],
                        [8.85157651e00, 4.70839103e00, 2.51314815e00, -5.64246393e00],
                        [7.54770787e00, 8.21372199e-02, 4.70293099e00, 3.47174970e00],
                        [-1.92174464e00, -5.91657547e00, 1.00629730e01, 2.62854305e01],
                    ],
                    [
                        [2.20347520e00, 3.00714687e00, 1.59377661e00, -6.41826692e-01],
                        [2.15604582e00, 1.86128202e00, 1.28260457e00, -1.03918888e00],
                        [1.50501488e00, 5.74547239e-01, 1.52092784e00, -3.94591487e-01],
                        [2.83614456e-02, -8.95222937e-01, 2.49176874e00, 1.81097696e00],
                    ],
                    [
                        [6.98668139e-01, 2.56635250e-01, 1.74332893e00, 3.79321436e-01],
                        [2.39593746e-01, 4.88748160e-01, 1.16884612e00, -7.54110131e-03],
                        [-6.40285805e-02, 5.82931602e-01, 4.67005716e-01, 3.76288542e-02],
                        [-2.10896883e-01, 5.17706856e-01, -4.13562541e-01, 6.96975860e-01],
                    ],
                ],
            ]
        )
    ...
    q1_truth = (
        np.array(
            [
                [
                    [
                        [-8.98245364e-13, 2.03803219e-13, 2.88874668e-12, 2.18043424e-12],
                ...
                [
                    [
                        ...
                        [-7.25812141e-12, 8.47072439e-13, -2.06762495e-12, -4.40132129e-12],
                    ],
                    [
                    ...
                    [
                        ...
                        [1.69339642e-13, 2.91716502e-13, -1.20043003e-14, 4.43770388e-13],
                    ],
                ],
            ]
        )
            ...
            [
                ...
                [
                    ...
                    [
                        ...
                        [-2.46502471e-13, -2.43524217e-13, -3.81250581e-13, -1.70270366e-13],
                        [8.12479206e-13, -1.38629628e-13, -8.05591138e-13, -7.80286006e-13],
                    ],
                    [
                        [-2.19626566e-13, -1.52852503e-13, 4.07706963e-13, 1.52323163e-12],
                        [-2.56235985e-13, -1.20817691e-13, 6.51260820e-15, 3.49591511e-13],
                        [-2.44063890e-13, -1.21871642e-13, -9.09798480e-14, -1.59903476e-13],
                        [-2.47929201e-13, -1.77269110e-13, -1.12991330e-13, -6.06795348e-13],
                    ],
                ],
                [
                    [
                    ...
                    [
                        ...
                        [-2.27182863e-13, -2.74607819e-13, -1.10587309e-13, -3.88915866e-13],
                    ],
                ],
            ]
        )
