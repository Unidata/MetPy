...
def test_moist_lapse(temp_units):
    ...
    true_temp = np.array([293, 284.64, 272.8, 264.41, 252.88]) * units.kelvin
...
def test_moist_lapse_ref_pressure():
    ...
    true_temp = np.array([294.76, 284.64, 272.8, 264.41, 252.88]) * units.kelvin
    assert_array_almost_equal(temp, true_temp, 2)
...
def test_parcel_profile():
    ...
    true_prof = np.array([303.15, 294.16, 288.01, 283.06, 277.04, 269.37,
                          258.92]) * units.kelvin
...
def test_parcel_profile_lcl():
    ...
    true_prof = np.array([297.35, 297.01, 294.48, 293.47, 292.9, 292.79, 289.77, 289.3,
...
def test_parcel_profile_saturated():
    ...
    true_prof = np.array([296.95, 284.35, 271.08]) * units.kelvin
...
def test_lcl_kelvin():
    ...
    assert_almost_equal(lcl_temperature, 262.802 * units.kelvin, 2)
...
def test_lcl_nans():
    ...
    assert_array_almost_equal(lcl_press, np.array([np.nan, 836.2231283,
                                                   np.nan, 836.2231283]) * units.hPa)
    assert_array_almost_equal(lcl_temp, np.array([np.nan, 18.8041938,
                                                  np.nan, 18.8041938]) * units.degC)
...
def test_ccl_basic():
    ...
    assert_almost_equal(ccl_p, 762.81930 * units.mbar, 5)
    assert_almost_equal(ccl_t, 15.41210 * units.degC, 5)
    assert_almost_equal(t_c, 37.99401 * units.degC, 5)
...
def test_ccl_unit():
    ...
    ccl_p, ccl_t, t_c = ccl(pressure, temperature, dewpoint)
    assert_almost_equal(ccl_p, (762.81930 * 100) * units.Pa, 3)
    assert_almost_equal(ccl_t, (15.41210 + 273.15) * units.kelvin, 3)
    assert_almost_equal(t_c, (37.99401 + 273.15) * units.kelvin, 3)
...
def test_multiple_ccl():
    ...
    assert_almost_equal(t_c, 30.8713506 * units.degC, 5)
    ...
    assert_almost_equal(t_c, 12.5032889 * units.degC, 5)
    ...
    assert_array_almost_equal(t_c, np.array([12.5032889, 30.8713506]) * units.degC, 5)
...
def test_ccl_with_ml():
    ...
    assert_array_almost_equal(ccl_p, np.array(
        [850.438506, 784.136599, 782.671594, 737.637719, 647.950662]) * units.mbar, 5)
    assert_array_almost_equal(ccl_t, np.array(
        [0.829278, -0.291727, -0.317423, -1.129214, -2.886271]) * units.degC, 5)
...
def test_lfc_inversion():
    ...
    lfc_pressure, lfc_temp = lfc(levels, temperatures, dewpoints)
    assert_almost_equal(lfc_pressure, 705.5807 * units.mbar, 2)
...
def test_sensitive_sounding():
    ...
    assert_almost_equal(pos, 0.002948 * units('J/kg'), 3)
    assert_almost_equal(neg, -286.683725 * units('J/kg'), 3)
...
def test_virtual_temperature_from_dewpoint():
    ...
    assert_almost_equal(tv, 33.6680 * units.degC, 3)
...
def test_el_kelvin():
    ...
    assert_almost_equal(el_pressure, 476.07144 * units.mbar, 3)
    assert_almost_equal(el_temp, -11.139 * units.degC, 3)
...
def test_cape_cin():
    ...
    assert_almost_equal(cape, 210.952968 * units('joule / kilogram'), 2)
    assert_almost_equal(cin, -10.37750324 * units('joule / kilogram'), 2)
...
def test_cape_cin_no_el():
    ...
    cape, cin = cape_cin(p, temperature, dewpoint, parcel_prof)
    assert_almost_equal(cape, 12.52109932 * units('joule / kilogram'), 2)
    assert_almost_equal(cin, -10.377503242 * units('joule / kilogram'), 2)
...
def test_surface_based_cape_cin(array_class):
    ...
    assert_almost_equal(cape, 210.95296816 * units('joule / kilogram'), 2)
    assert_almost_equal(cin, -33.6620801701 * units('joule / kilogram'), 2)
...
def test_most_unstable_cape_cin_surface():
    ...
    dewpoint = np.array([19., -11.2, -10.8, -10.4, -10., -53.2]) * units.celsius
    mucape, mucin = most_unstable_cape_cin(pressure, temperature, dewpoint)
    assert_almost_equal(mucape, 210.95296816 * units('joule / kilogram'), 2)
    assert_almost_equal(mucin, -33.6620801701 * units('joule / kilogram'), 2)
...
def test_most_unstable_cape_cin():
    ...
    mucape, mucin = most_unstable_cape_cin(pressure, temperature, dewpoint)
    assert_almost_equal(mucape, 173.492800262 * units('joule / kilogram'), 4)
    assert_almost_equal(mucin, -21.159922894 * units('joule / kilogram'), 4)
...
def test_mixed_layer_cape_cin(multiple_intersections):
    ...
    assert_almost_equal(mlcape, 1120.723057388 * units('joule / kilogram'), 2)
    assert_almost_equal(mlcin, -13.6568786318 * units('joule / kilogram'), 2)
...
def test_mixed_layer_cape_cin_bottom_pressure(multiple_intersections):
    ...
    assert_almost_equal(mlcape_middle, 1166.88 * units('joule / kilogram'), 2)
    assert_almost_equal(mlcin_middle, -37.71 * units('joule / kilogram'), 2)
...
def test_dewpoint_specific_humidity_xarray(index_xarray_data):
    ...
    td = dewpoint_from_specific_humidity(p, specific_humidity=q)
    assert_array_almost_equal(td, index_xarray_data.dewpoint, decimal=1)
...
def test_multiple_lfcs_simple(multiple_intersections):
    ...
    assert_almost_equal(lfc_temp_bottom, 13.92200100 * units.degC, 3)
...
def test_multiple_lfs_wide(multiple_intersections):
    ...
    assert_almost_equal(lfc_temp_wide, 4.8516747 * units.degC, 3)
...
def test_multiple_els_simple(multiple_intersections):
    ...
    assert_almost_equal(el_temp_top, -56.806956 * units.degC, 3)
    assert_almost_equal(el_pressure_bottom, 849.85394 * units.mbar, 3)
    assert_almost_equal(el_temp_bottom, 12.4165561 * units.degC, 3)
...
def test_multiple_el_wide(multiple_intersections):
    ...
    assert_almost_equal(el_temp_wide, -56.8069560 * units.degC, 3)
...
def test_el_lfc_most_cape_bottom():
    ...
    assert_almost_equal(lfc_temp, 5.39186322 * units.degC, 3)
    assert_almost_equal(el_pressure, 659.284276 * units.hPa, 3)
    assert_almost_equal(el_temp, 1.98172061 * units.degC, 3)
...
def test_cape_cin_top_el_lfc(multiple_intersections):
    ...
    assert_almost_equal(cape, 1335.95888464 * units('joule / kilogram'), 3)
    assert_almost_equal(cin, -35.816974306 * units('joule / kilogram'), 3)
...
def test_cape_cin_bottom_el_lfc(multiple_intersections):
    ...
    assert_almost_equal(cape, 4.43749259 * units('joule / kilogram'), 3)
    assert_almost_equal(cin, -6.08659801232 * units('joule / kilogram'), 3)
...
def test_cape_cin_wide_el_lfc(multiple_intersections):
    ...
    cape, cin = cape_cin(levels, temperatures, dewpoints, parcel_prof, which_lfc='wide',
                         which_el='wide')
    assert_almost_equal(cape, 1335.9588846 * units('joule / kilogram'), 3)
    assert_almost_equal(cin, -35.816974306 * units('joule / kilogram'), 3)
...
def test_cape_cin_custom_profile():
    ...
    cape, cin = cape_cin(p, temperature, dewpoint, parcel_prof)
    assert_almost_equal(cape, 1641.74927485 * units('joule / kilogram'), 2)
    assert_almost_equal(cin, 0.0 * units('joule / kilogram'), 2)
...
def test_cape_cin_value_error():
    ...
    expected_cape, expected_cin = 2088.286477 * units('joules/kg'), 0.0 * units('joules/kg')
...
def test_lcl_grid_surface_lcls():
    ...
    pres_truth = np.array([999.87072, 932.08387, 1009.89001]) * units.hPa
...
def test_parcel_profile_drop_duplicates():
    ...
    truth = np.array([292.75, 291.78965, 291.11868, 290.61062,
                      289.92716, 289.83344, 289.35192, 288.55244,
                      135.44123, 134.9707, 131.25278, 131.04968,
                      125.91971, 123.61908, 120.83366, 120.83366,
                      119.18549]) * units.kelvin
