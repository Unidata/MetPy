...
def test_cross_xy():
    ...
    ds = xr.Dataset(
        ...
        coords={
            "time": xr.DataArray(
                np.array([np.datetime64("2018-07-01T00:00")]), name="time", dims=["time"]
            ),
            "isobaric": xr.DataArray(
                np.linspace(1000, 500, 5),
                name="isobaric",
            ...
            "y": xr.DataArray(
                np.linspace(-1500, 0, 6), name="y", dims=["y"], attrs={"units": "km"}
            ),
            "x": xr.DataArray(
                np.linspace(-500, 3000, 7), name="x", dims=["x"], attrs={"units": "km"}
            ),
        },
    )
    ds["u_wind"].attrs = ds["v_wind"].attrs = {"grid_mapping": "lambert_conformal"}
...
def test_latitude_from_cross_section_given_y(test_cross_xy):
    ...
    true_latitude_values = np.array(
        [36.46, 38.64829115, 40.44833152, 41.81277354, 42.7011178, 43.0845549, 42.95]
    )
    index = xr.DataArray(range(7), name="index", dims=["index"])
...
def test_unit_vectors_from_cross_section_given_xy(test_cross_xy):
    ...
    true_unit_tangent = np.array(
        [
            [0.93567585, 0.929688, 0.92380315, 0.91844706, 0.91349795, 0.90875771, 0.90400673],
            [
                0.35286074,
                0.36834796,
                0.3828678,
...
def test_cross_section_components(test_cross_lonlat):
    ...
    tang_wind, norm_wind = cross_section_components(
        test_cross_lonlat["u_wind"], test_cross_lonlat["v_wind"]
    )
    ...
    true_tang_wind = xr.DataArray(
        true_tang_wind_values * units.knots,
        coords=test_cross_lonlat["u_wind"].coords,
        dims=test_cross_lonlat["u_wind"].dims,
        attrs=test_cross_lonlat["u_wind"].attrs,
    )
    true_norm_wind = xr.DataArray(
        true_norm_wind_values * units.knots,
        coords=test_cross_lonlat["u_wind"].coords,
        dims=test_cross_lonlat["u_wind"].dims,
        attrs=test_cross_lonlat["u_wind"].attrs,
    )
...
def test_tangential_component(test_cross_xy):
    ...
    true_tang_wind = xr.DataArray(
        true_tang_wind_values * units("m/s"),
...
def test_normal_component(test_cross_xy):
    ...
    true_norm_wind = xr.DataArray(
        true_norm_wind_values * units("m/s"),
        coords=test_cross_xy["u_wind"].coords,
        dims=test_cross_xy["u_wind"].dims,
        attrs=test_cross_xy["u_wind"].attrs,
    )
...
def test_absolute_momentum_given_lonlat(test_cross_lonlat):
    ...
    true_momentum = xr.DataArray(
        true_momentum_values * units("m/s"),
        coords=test_cross_lonlat["u_wind"].coords,
        dims=test_cross_lonlat["u_wind"].dims,
    )
