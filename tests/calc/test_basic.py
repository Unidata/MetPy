...
def test_wind_comps_basic():
    ...
    speed = np.array([4, 4, 4, 4, 25, 25, 25, 25, 10.0]) * units.mph
...
def test_speed():
    ...
    u = np.array([4.0, 2.0, 0.0, 0.0]) * units("m/s")
    v = np.array([0.0, 2.0, 4.0, 0.0]) * units("m/s")

    speed = wind_speed(u, v)

    s2 = np.sqrt(2.0)
    true_speed = np.array([4.0, 2 * s2, 4.0, 0.0]) * units("m/s")
...
def test_direction():
    ...
    true_dir = np.array([270.0, 225.0, 180.0, 0.0]) * units.deg
...
def test_direction_masked():
    ...
    true_dir = np.array([270.0, 225.0, 180.0, 0.0])
...
def test_direction_with_north_and_calm():
    ...
    u = np.array([0.0, -0.0, 0.0]) * units("m/s")
    v = np.array([0.0, 0.0, -5.0]) * units("m/s")

    direc = wind_direction(u, v)

    true_dir = np.array([0.0, 0.0, 360.0]) * units.deg
...
def test_speed_direction_roundtrip():
    ...
    wspd = np.array([15.0, 5.0, 2.0, 10.0]) * units.meters / units.seconds
    wdir = np.array([160.0, 30.0, 225.0, 350.0]) * units.degrees
...
def test_windchill_invalid():
    ...
    truth = units.Quantity(
        np.ma.array(
            [2.6230789, np.nan, np.nan, np.nan, np.nan, np.nan],
...
def test_heat_index_vs_nws():
    ...
    hi = heat_index(temp, rh)
    truth = units.Quantity(
        np.ma.array([87, 121, 40, 116], mask=[False, False, True, False]), units.degF
    )
...
def test_height_to_geopotential():
    ...
    assert_array_almost_equal(
        geopot, units.Quantity([0.0, 9805, 19607, 29406], units("m**2 / second**2")), 0
    )
...
def test_geopotential_to_height():
    ...
    geopotential = units.Quantity(
        [0.0, 9805.11102602, 19607.14506998, 29406.10358006], units("m**2 / second**2")
    )
...
class TestIrrad(object):
    def test_basic(self):
        ...
        values = np.array([0.0, 344.1, 682.6, 933.9, 1067.6, 1067.6, 933.9, 682.6, 344.1, 0.0])
...
def test_pressure_to_heights_basic():
    ...
    pressures = np.array([975.2, 987.5, 956.0, 943.0]) * units.mbar
...
def test_heights_to_pressure_basic():
    ...
    values = np.array([975.2, 987.5, 956.0, 943.0]) * units.mbar
...
def test_coriolis_force():
    ...
    lat = np.array([-90.0, -30.0, 0.0, 30.0, 90.0]) * units.degrees
    cor = coriolis_parameter(lat)
    values = np.array([-1.4584232e-4, -0.72921159e-4, 0, 0.72921159e-4, 1.4584232e-4]) * units(
        "s^-1"
    )
...
def test_sigma_to_pressure():
    ...
    sigma = np.arange(0.0, 1.1, 0.1)
    expected = np.arange(0.0, 1100.0, 100.0) * units.hPa
...
def test_warning_dir():
    ...
    with pytest.warns(UserWarning):
        wind_components(3.0 * units("m/s"), 270)
...
def test_apparent_temperature():
    ...
    truth = units.Quantity(
        np.ma.array(
            [[99.6777178, 86.3357671, 70], [8.8140662, 20, 60]],
...
def test_smooth_gaussian_small_n():
    ...
    s_true = [
        [0.0141798077, 1.02126971, 4.02126971, 9.02126971, 15.9574606],
...
def test_smooth_n_pt_5():
    ...
    hght = np.array(
        [
            [5640.0, 5640.0, 5640.0, 5640.0, 5640.0],
...
def test_smooth_n_pt_3d_units():
    ...
    hght = [
        ...
        [
            ...
            [6979.2, 6940.8, 6892.8, 6859.2, 6820.8],
        ],
    ] * units.m
    shght = smooth_n_point(hght, 9, 2)
    s_true = [
...
def test_smooth_gaussian_temperature():
    ...
    smooth_t_true = (
        np.array(
            [
                [2.8892, 3.7657, 6.2805, 6.8532, 5.3174],
...
def test_smooth_window_1d_dataarray():
    ...
    temperature = xr.DataArray(
        [37.0, 32.0, 34.0, 29.0, 28.0, 24.0, 26.0, 24.0, 27.0, 30.0],
        dims=("time",),
