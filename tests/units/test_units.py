...
from metpy.testing import (  # noqa: F401
...
test_funcs = [
    check_units(
        "[temperature]", "[pressure]", dens="[mass]/[volume]", mixing="[dimensionless]"
    )(unit_calc),
    check_units(
        temp="[temperature]",
        press="[pressure]",
    ...
    check_units("[temperature]", "[pressure]", "[mass]/[volume]", "[dimensionless]")(
        unit_calc
    ),
]
...
def test_good_units(func):
    ...
    func(30 * units.degC, 1000 * units.mbar, 1.0 * units("kg/m^3"), 1, 5.0)


test_params = [
    (
        (30 * units.degC, 1000 * units.mb, 1 * units("kg/m^3"), 1, 5 * units("J/kg")),
        {},
