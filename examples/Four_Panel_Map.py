...
def plot_background(ax):
    ax.set_extent([235.0, 290.0, 20.0, 55.0])
...
fig, axarr = plt.subplots(
    nrows=2, ncols=2, figsize=(20, 13), constrained_layout=True, subplot_kw={"projection": crs}
)
...
c1 = axlist[0].contour(
    lon_2d, lat_2d, heights_300, colors="black", linewidths=2, transform=ccrs.PlateCarree()
)
...
c2 = axlist[1].contour(
    lon_2d, lat_2d, heights_500, colors="k", linewidths=2, transform=ccrs.PlateCarree()
)
...
cf3 = axlist[2].contourf(
    lon_2d, lat_2d, surface_temp, cmap="YlOrRd", transform=ccrs.PlateCarree(), zorder=0
)
...
cf4 = axlist[3].contourf(
    lon_2d, lat_2d, precip_water, cmap="Greens", transform=ccrs.PlateCarree(), zorder=0
)
...
fig.set_constrained_layout_pads(w_pad=0.0, h_pad=0.1, hspace=0.0, wspace=0.0)
