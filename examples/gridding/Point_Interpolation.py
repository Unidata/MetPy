...
def station_test_data(variable_names, proj_from=None, proj_to=None):
    ...
    data = np.concatenate(
        [
            all_data[all_stids.index(site)].reshape(
                1,
            )
            for site in all_stids
        ]
    )
...
fig.colorbar(mmb, shrink=0.4, pad=0, boundaries=levels)
...
gx, gy, img = interpolate_to_grid(
    x, y, temp, interp_type="cressman", minimum_neighbors=1, hres=75000, search_radius=100000
)
img = np.ma.masked_where(np.isnan(img), img)
...
gx, gy, img1 = interpolate_to_grid(
    x, y, temp, interp_type="barnes", hres=75000, search_radius=100000
)
...
# linear
gx, gy, img = interpolate_to_grid(
    x, y, temp, interp_type="rbf", hres=75000, rbf_func="linear", rbf_smooth=0
)
img = np.ma.masked_where(np.isnan(img), img)
