...
to_proj = ccrs.AlbersEqualArea(central_longitude=-97.0, central_latitude=38.0)
...
with get_test_data("station_data.txt") as f:
    data = pd.read_csv(
        ...
        names=["latitude", "longitude", "slp", "temperature", "wind_dir", "wind_speed"],
        na_values=-99999,
    )
...
windgridx, windgridy, uwind = interpolate_to_grid(
    x_masked, y_masked, np.array(u), interp_type="cressman", search_radius=400000, hres=100000
)

_, _, vwind = interpolate_to_grid(
    x_masked, y_masked, np.array(v), interp_type="cressman", search_radius=400000, hres=100000
)
...
fig.colorbar(mmb, shrink=0.4, pad=0.02, boundaries=levels)

view.barbs(windgridx, windgridy, uwind, vwind, alpha=0.4, length=5)
