...
u, v = wind_components(
    data_arr["wind_speed"].values * units("m/s"), data_arr["wind_dir"].values * units.degree
)
...
proj = ccrs.LambertConformal(
    central_longitude=-95, central_latitude=35, standard_parallels=[35]
)
...
stationplot = StationPlot(
    ax, data["longitude"], data["latitude"], transform=ccrs.PlateCarree(), fontsize=12
)
...
custom_layout.add_value(
    "SW", "dew_point_temperature", fmt=".1f", units="degF", color="darkgreen"
)
