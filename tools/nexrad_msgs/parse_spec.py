    ...
    with open(fname, "r") as infile:
        ...
        for lineno, line in enumerate(infile):
            ...
            try:
                ...
                size = end - start + 1
                assert size >= 4
                fmt = fix_type(typ, size, additional=[("See Note (5)", ("{size}s", 1172))])
...
def fix_type(typ, size, additional=None):
    ...
    for t, info in my_types:
        ...
        if matches:
            ...
            assert size == true_size, "{}: Got size {} instead of {}".format(
                typ, size, true_size
            )
...
def write_file(fname, info):
    ...
    with open(fname, "w") as outfile:
        ...
        outdata = ",\n                ".join(
            '"{name}": "{desc}"'.format(**i) for i in info if need_desc(i)
        )
        ...
        outdata = ",\n          ".join(
            '({fname}, "{fmt}")'.format(fname=field_name(i), **i) for i in info
        )
        outfile.write(outdata)
        outfile.write("]\n")
