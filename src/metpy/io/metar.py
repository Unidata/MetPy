...
def parse_metar_to_dataframe(metar_text, *, year=None, month=None):
    ...
    df = pd.DataFrame(
        {
            "station_id": metar_vars.station_id,
    ...
    try:
        ...
        df["air_pressure_at_sea_level"] = float(
    ...
    df["eastward_wind"], df["northward_wind"] = wind_components(
        (df.wind_speed.values * units.kts), df.wind_direction.values * units.degree
    )
...
def parse_metar_to_named_tuple(metar_text, station_metadata, year, month):
    ...
    if ("OVC" or "VV") in tree.skyc.text:
        ...
        ("SKC" in tree.skyc.text)
        or ("NCD" in tree.skyc.text)
        or ("NSC" in tree.skyc.text)
...
def parse_metar_file(filename, *, year=None, month=None):
    ...
    df["air_pressure_at_sea_level"] = (
        altimeter_to_sea_level_pressure(
            altim * units("inHg"), elev * units("meters"), temp * units("degC")
        )
        .to("hPa")
        .magnitude
    )
