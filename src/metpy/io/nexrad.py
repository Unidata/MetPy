...
def bzip_blocks_decompress_all(data):
    ...
    while offset < len(data):
        ...
        try:
            ...
            if frames:
                logging.warning(
                    "Error decompressing bz2 block stream at offset: %d", offset - 4
                )
                break
...
class Level2File:
    ...
    msg_hdr_fmt = NamedStruct(
        [
            ...
            (
                "rda_channel",
                "B",
                BitField("Redundant Channel 1", "Redundant Channel 2", None, "ORDA"),
            ),
            ("msg_type", "B"),
            ("seq_num", "H"),
            ("date", "H"),
    ...
    def _read_data(self):
        ...
        while not self._buffer.at_end():
            ...
            if msg_hdr.size_hw:
                ...
                if msg_hdr.size_hw == 65535:
                    msg_bytes = (
                        msg_hdr.num_segments << 16 | msg_hdr.segment_num + self.CTM_HEADER_SIZE
                    )
        ...
        if self._msg_buf:
            log.warning(
                "Remaining buffered messages segments for message type(s): %s",
    ...
    msg1_data_hdr = namedtuple(
        "Msg1DataHdr", "name first_gate gate_width num_gates scale offset"
    )
    ...
    msg2_fmt = NamedStruct(
        [
            ...
            (
                "control_status",
                "H",
                BitField("None", "Local Only", "RPG (Remote) Only", "Either"),
            ),
            (
                "aux_power_gen_state",
                "H",
            ...
            (
                ...
                "H",
                BitField("None", "None", "Reflectivity", "Velocity", "Width"),
            ),
            ("vcp_num", "h"),
            (
                "rda_control_auth",
                "H",
                BitField("No Action", "Local Control Requested", "Remote Control Enabled"),
            ),
            ("rda_build", "H", version),
    ...
    def _decode_msg2(self, msg_hdr):
        ...
        remaining = (
            msg_hdr.size_hw * 2 - self.msg_hdr_fmt.size - self._buffer.offset_from(msg_start)
        )
        ...
        if remaining:
            log.info(
                "Padding detected in message 2. Length encoded as %d but offset when "
                "done is %d",
                2 * msg_hdr.size_hw,
    ...
    def _decode_msg5(self, msg_hdr):
        ...
        self._check_size(
            msg_hdr, self.vcp_fmt.size + vcp_info.num_el_cuts * self.vcp_el_fmt.size
        )
    ...
    def _decode_msg13(self, msg_hdr):
        ...
        if data:
            ...
            for e in range(num_el):
                ...
                if seg_num != (e + 1):
                    log.warning(
                        "Message 13 segments out of sync -- read {} but on {}".format(
                            seg_num, e + 1
                        )
                    )
    ...
    msg31_data_hdr_fmt = NamedStruct(
        [
            ("stid", "4s"),
            ("time_ms", "L"),
            ("date", "H"),
    ...
    msg31_vol_const_fmt = NamedStruct(
        [
            ("type", "s"),
            ("name", "3s"),
            ("size", "H"),
    ...
    msg31_el_const_fmt = NamedStruct(
        [
            ("type", "s"),
            ("name", "3s"),
            ("size", "H"),
    ...
    def _decode_msg31(self, msg_hdr):
        ...
        for ptr in self._buffer.read_binary(data_hdr.num_data_blks, ">L"):
            if ptr:
                ...
                if info.startswith(b"RVOL"):
                    ...
                    vals = self._buffer.read_array(
                        count=hdr.num_gates, dtype=f">u{hdr.data_size // 8}"
                    )
        ...
        if data_hdr.num_data_blks != block_count:
            log.warning(
                "Incorrect number of blocks detected -- Got %d" " instead of %d",
                block_count,
                data_hdr.num_data_blks,
            )

        if data_hdr.rad_length != self._buffer.offset_from(msg_start):
            log.info(
                "Padding detected in message. Length encoded as %d but offset when "
                "done is %d",
                data_hdr.rad_length,
                self._buffer.offset_from(msg_start),
            )
    ...
    def _buffer_segment(self, msg_hdr):
        ...
        bufs[msg_hdr.segment_num] = self._buffer.read(
            2 * msg_hdr.size_hw - self.msg_hdr_fmt.size
        )

        # Warn for badly formatted data
        if len(bufs) != msg_hdr.segment_num:
            log.warning(
                "Segment out of order (Got: %d Count: %d) for message type %d.",
                msg_hdr.segment_num,
                len(bufs),
                msg_hdr.msg_type,
            )
    ...
    def _add_sweep(self, hdr):
        ...
        if len(self.sweeps) != hdr.el_num:
            log.warning(
                "Missed elevation -- Have %d but data on %d." " Compensating...",
                len(self.sweeps),
                hdr.el_num,
            )
    ...
    def _check_size(self, msg_hdr, size):
        ...
        if size != hdr_size:
            log.warning(
                "Message type %d should be %d bytes but got %d",
                msg_hdr.msg_type,
                size,
                hdr_size,
            )
...
def supplemental_scan(ind):
    ...
    def inner(seq):
        ...
        return {0: "Non-supplemental scan", 2: "SAILS scan", 1: "MRLE scan"}.get(
            seq[ind] & 0x001F, "Unknown"
        )
...
class EDRMapper(DataMapper):
    ...
    def __init__(self, prod):
        ...
        scale = prod.thresholds[0] / 1000.0
        offset = prod.thresholds[1] / 1000.0
...
class Level3File:
    ...
    gsm_fmt = NamedStruct(
        [
            ...
            (
                ...
                "h",
                BitField(
                    "Spare", "Startup", "Standby", "Restart", "Operate", "Off-line Operate"
                ),
            ),
            (
                "rda_alarms",
            ...
            (
                ...
                "h",
                BitField(
                    "Spare", "None", "Reflectivity", "Velocity", "Spectrum Width", "Dual Pol"
                ),
            ),
            (
                "rpg_op_status",
            ...
            (
                ...
                BitField(
                    ...
                    "Spare",
                    "Product Storage Loadshed" "Spare",
                    "Spare",
                    "Spare",
            ...
            (
                ...
                "h",
                BitField("Commanded Disconnect", "Narrowband Loadshed"),
            ),
            ("h_ref_calib", "h", scaler(0.25)),
            (
                "prod_avail",
                "h",
                BitField("Product Availability", "Degraded Availability", "Not Available"),
            ),
    ...
    sym_block_fmt = NamedStruct(
        [("divider", "h"), ("block_id", "h"), ("block_len", "L"), ("nlayer", "H")],
        ">",
        "SymBlock",
    )
    tab_header_fmt = NamedStruct(
        [("divider", "h"), ("block_id", "h"), ("block_len", "L")], ">", "TabHeader"
    )
    tab_block_fmt = NamedStruct([("divider", "h"), ("num_pages", "h")], ">", "TabBlock")
    sym_layer_fmt = NamedStruct([("divider", "h"), ("length", "L")], ">", "SymLayer")
    graph_block_fmt = NamedStruct(
        [("divider", "h"), ("block_id", "h"), ("block_len", "L"), ("num_pages", "H")],
        ">",
        "GraphBlock",
    )
    ...
    prod_spec_map = {
        ...
        41: (
            ...
            230.0,
            LegacyMapper,
            (("el_angle", scaled_elem(2, 0.1)), ("max", scaled_elem(3, 1000))),
        ),  # Max in ft
        48: (
            "VAD Wind Profile",
            None,
            LegacyMapper,
            (("max", 3), ("dir_max", 4), ("alt_max", scaled_elem(5, 10))),
        ),  # Max in ft
        50: (
        ...
        58: ("Storm Tracking Information", 460.0, LegacyMapper, (("num_storms", 3),)),
        59: ("Hail Index", 230.0, LegacyMapper, ()),
        61: (
        ...
        62: ("Storm Structure", 460.0, LegacyMapper, ()),
        63: (
            ...
            (
                ("max", 3),
                ("layer_bottom", scaled_elem(4, 1000.0)),
                ("layer_top", scaled_elem(5, 1000.0)),
                ("calib_const", float_elem(7, 8)),
            ),
        ),
        64: (
        ...
        74: ("Radar Coded Message", 460.0, LegacyMapper, ()),
        78: (
        ...
        93: (
            ...
            115.0,
            DigitalVelMapper,
            (("el_angle", scaled_elem(2, 0.1)), ("min", 3), ("max", 4), ("precision", 6)),
        ),
        94: (
        ...
        135: (
            ...
            (
                ("el_angle", scaled_elem(2, 0.1)),
                ("max", scaled_elem(3, 1000.0)),  # Max in ft
                ("num_edited", 4),
                ("ref_thresh", 5),
                ("points_removed", 6),
                ("compression", 7),
        ...
        167: (
            "Super Res Digital Correlation Coefficient",
            300.0,
            GenericDigitalMapper,
            (
                ("el_angle", scaled_elem(2, 0.1)),
    ...
    def __init__(self, filename):
        ...
        log.debug(
            "Buffer size: %d (%d expected) Header: %s",
            len(self._buffer),
            self.header.msg_len,
            self.header,
        )
        ...
        if not self._buffer.check_remains(self.header.msg_len - self.header_fmt.size):
            log.warning(
                "Product contains an unexpected amount of data remaining--have: %d "
        ...
        self.metadata["msg_time"] = nexrad_to_datetime(
            self.header.date, self.header.time * 1000
        )
        self.metadata["vol_time"] = nexrad_to_datetime(
            self.prod_desc.vol_date, self.prod_desc.vol_start_time * 1000
        )
        self.metadata["prod_time"] = nexrad_to_datetime(
            self.prod_desc.prod_gen_date, self.prod_desc.prod_gen_time * 1000
        )
        ...
        log.debug(
            "Product info--name: %s max_range: %f mapper: %s metadata: %s",
            self.product_name,
            self.max_range,
            mapper,
            meta,
        ...
        if self.header.code in self.standalone_tabular:
            ...
            if self.prod_desc.graph_off:
                ...
                # it's not clear why.
                self._unpack_standalone_graphblock(
                    msg_start, 2 * (self.prod_desc.graph_off - 1)
                )
        ...
        if "defaultVals" in self.metadata:
            log.warning(
                "%s: Using default metadata for product %d", self.filename, self.header.code
            )
    ...
    def _unpack_symblock(self, start, offset):
        ...
        self.sym_block = []
        assert blk.divider == -1, "Bad divider for symbology block: {:d} should be -1".format(
            blk.divider
        )
        assert blk.block_id == 1, "Bad block ID for symbology block: {:d} should be 1".format(
            blk.block_id
        )
        for _ in range(blk.nlayer):
            ...
            while self._buffer.offset_from(layer_start) < layer_hdr.length:
                ...
                if packet_code in self.packet_map:
                    ...
                    log.warning(
                        "%s: Unknown symbology packet type %d/%x.",
                        self.filename,
                        packet_code,
                        packet_code,
                    )
    ...
    def _unpack_graphblock(self, start, offset):
        ...
        assert hdr.divider == -1, "Bad divider for graphical block: {:d} should be -1".format(
            hdr.divider
        )
        assert hdr.block_id == 2, "Bad block ID for graphical block: {:d} should be 1".format(
            hdr.block_id
        )
        self.graph_pages = []
        for page in range(hdr.num_pages):
            ...
            while self._buffer.offset_from(page_start) < page_size:
                ...
                if packet_code in self.packet_map:
                    ...
                    log.warning(
                        "%s: Unknown graphical packet type %d/%x.",
                        self.filename,
                        packet_code,
                        packet_code,
                    )
    ...
    def _unpack_standalone_graphblock(self, start, offset):
        ...
        while not self._buffer.at_end():
            ...
            if packet_code in self.packet_map:
                ...
                log.warning(
                    "%s: Unknown standalone graphical packet type %d/%x.",
                    self.filename,
                    packet_code,
                    packet_code,
                )
    ...
    def _unpack_packet_radial_data(self, code, in_sym_block):
        ...
        return {
            ...
            "data": list(vals),
            "center": (
                hdr.i_center * self.pos_scale(in_sym_block),
    ...
    digital_radial_hdr_fmt = NamedStruct(
        [
            ("ind_first_bin", "H"),
            ("nbins", "H"),
            ("i_center", "h"),
    ...
    digital_radial_fmt = NamedStruct(
        [("num_bytes", "H"), ("start_angle", "h"), ("angle_delta", "h")],
        ">",
        "DigitalRadialData",
    )
    ...
    def _unpack_packet_uniform_text(self, code, in_sym_block):
        ...
        return {
            "x": i_start * self.pos_scale(in_sym_block),
            "y": j_start * self.pos_scale(in_sym_block),
            "color": value,
            "text": text,
        }
    ...
    def _unpack_packet_special_graphic_symbol(self, code, in_sym_block):
        ...
        while self._buffer.offset_from(packet_data_start) < num_bytes:
            ...
            if code in (3, 11, 25):
                ...
                if kind not in point_feature_map:
                    log.warning(
                        "%s: Unknown graphic symbol point kind %d/%x.",
                        self.filename,
                        kind,
                        kind,
                    )
        ...
        if code != 20:
            if code not in type_map:
                log.warning(
                    "%s: Unknown graphic symbol type %d/%x.", self.filename, code, code
                )
                ret["type"] = "Unknown"
            else:
    ...
    def _unpack_packet_scit(self, code, in_sym_block):
        ...
        while self._buffer.offset_from(packet_data_start) < num_bytes:
            ...
            if next_code not in self.packet_map:
                log.warning(
                    "%s: Unknown packet in SCIT %d/%x.", self.filename, next_code, next_code
                )
                ...
                if next_code == 6:
                    ...
                    log.warning(
                        "%s: Unsupported packet in SCIT %d/%x.",
                        self.filename,
                        next_code,
                        next_code,
                    )
    ...
    def _unpack_packet_wind_barbs(self, code, in_sym_block):
        ...
        while self._buffer.offset_from(packet_data_start) < num_bytes:
            ...
            ret["x"].append(
                self._buffer.read_int(2, "big", signed=True) * self.pos_scale(in_sym_block)
            )
            ret["y"].append(
                self._buffer.read_int(2, "big", signed=True) * self.pos_scale(in_sym_block)
            )
...
class Level3XDRParser(Unpacker):
    ...
    radial_fmt = namedtuple(
        "RadialComponent", ["description", "gate_width", "first_gate", "parameters", "radials"]
    )
    radial_data_fmt = namedtuple(
        "RadialData", ["azimuth", "elevation", "width", "num_bins", "attributes", "data"]
    )
