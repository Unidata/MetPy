...
# with a list of integers in square brackets (e.g., [5400, 5460, 5520, 5580, 5640, 5700]) or
# programmatically (e.g., list(range(0, 10000, 60))). The second method is a way to easily
# set a contour interval (in this case 60).
#
# ``clabel``
#
...
# ``projection``
#
# The projection can be set with the name of a default projection (‘lcc’, ‘mer’, or ‘ps’) or
# it can be set to a Cartopy projection.
#
# ``layers``
#
# This attribute will add map layers to identify boundaries or features to plot on the map.
# Valid layers are borders, coastline, states, lakes, land, ocean, rivers, counties.
#
# ``area``
#
# This attribute sets the geographical area of the panel. This can be set with a predefined
# name of an area including all US state postal abbreviations (e.g., ‘us’, ‘natl’, ‘in’, ‘il’,
# ‘wi’, ‘mi’, etc.) or a tuple value that corresponds to longitude/latitude box based on the
# projection of the map with the format (west-most longitude, east-most longitude, south-most
# latitude, north-most latitude). This tuple defines a box from the lower-left to the
# upper-right corner.
#
# ``title``
#
...
# Set the attributes for the map and put the contours on the map
panel = MapPanel()
...
# Set the atrributes for plotting wind barbs
barbs = BarbPlot()
barbs.data = ds
...
# syntax. Whether it is surface or upper-air data, the PlotObs() class is what you would want
# to use. Then you would add those observations to a map panel and then collect the panels
...
# are current_weather, sky_cover, low_clouds, mid_clouds, high_clouds, and pressure_tendency.
#
# ``colors``
#
